package org.farka.taskmanager.web.rest;
...
import org.farka.taskmanager.config.EmbeddedKafka;
...
class TaskManagerKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/task-manager-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/task-manager-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/task-manager-kafka/unregister"));
                return;
            }
        }
